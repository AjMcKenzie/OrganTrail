...
namespace OrganTrail
{
    public partial class HuntingGame : Form
    {
        ...
        private void gameEngine(object sender, EventArgs e)
        {
            ...
            foreach (Control x in this.Controls)
            {
                ...
                if (x is PictureBox && x.Tag == "ammo")
                {
                    ...
                    if (((PictureBox)x).Bounds.IntersectsWith(picPlayer.Bounds))
                    {
                        ...
                        ((PictureBox)x).Dispose(); // dispose the picture box comletely from the program
            ...
            // below is the if statement which will be checking if the player hits a zombies

            if (x is PictureBox && x.Tag == "zombies")
            {
                ...
                // move zombie toward the player picture box

                if (((PictureBox)x).Left > picPlayer.Left)
                {
                    ((PictureBox)x).Left -= zombiesSpeed; // move zombie toward the left of the player
                    ((PictureBox)x).Image = Properties.Resources.zleft; // change the zombie image to the left
                }

                if (((PictureBox)x).Top > picPlayer.Top)
                {
                    ((PictureBox)x).Top -= zombiesSpeed; // move zombie upward the player top
                    ((PictureBox)x).Image = Properties.Resources.zup; // change the zombie picture to the top pointing image
                }

                if (((PictureBox)x).Left < picPlayer.Left)
                {
                    ((PictureBox)x).Left += zombiesSpeed; // move zombie towards the right of the player
                    ((PictureBox)x).Image = Properties.Resources.zright; // change the image to the right image
                }

                if (((PictureBox)x).Top < picPlayer.Top)
                {
                    ((PictureBox)x).Top += zombiesSpeed; // move the zombie toward the bottom of the player
        ...
        private void DropAmmo()
        {
            ...
            picPlayer.BringToFront(); // bring the player to front
